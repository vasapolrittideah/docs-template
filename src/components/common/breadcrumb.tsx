...
function BreadcrumbRoot({ asChild, children, className, ...rest }: BreadcrumbRootProps) {
  ...
  return (
    <Component className={cn('flex flex-wrap items-center gap-1.5', className)} {...rest}>
      {children}
    </Component>
  );
}
...
function BreadcrumbItemIcon<T extends React.ElementType>({ className, as, ...rest }: PolymorphicComponentProps<T>) {
...
function BreadcrumbItemArrowIcon<T extends React.ElementType>({
  ...
  return (
    <Component
      className={cn('text-text-disabled-300 flex size-5 items-center justify-center select-none', className)}
      {...rest}
    />
  );
}
